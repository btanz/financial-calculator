...
exports.homesave = function(inputs){
  ...
  function compute(data){
    ...
    result._1.finalsavingswohnungsbau   = _.extend(_.findWhere(data[0].results_1,{name: 'finalsavingswohnungsbau'}), {"value": helper.finalsavingswohnungsbau});
    //result._1.finalsavingswohnungsbau = _.extend(localElems['finalsavingswohnungsbau'], {"value": helper.finalsavingswohnungsbau});
    result._1.totalpays                 = _.extend(_.findWhere(data[0].results_1,{name: 'totalpays'}), {"value": helper.totalpays});
    ...
    result._1.wohnungsbau               = _.extend(_.findWhere(data[0].results_1,{name: 'wohnungsbau'}), {"value": helper.wohnungsbau});
    //result._1.wohnungsbau             = _.extend(localElems['wohnungsbau'],             {"value": helper.wohnungsbau});

    if(inputs.initialpay != 0){ result._1.initialpay = _.extend(_.findWhere(data[0].results_1,{name: 'initialpay'}), {"value": inputs.initialpay}); };
    //if(inputs.initialpay != 0){ result._1.initialpay               = _.extend(localElems['initialpay'],             {"value": inputs.initialpay}); };
    if(inputs.initialfee != 0){ result._1.initialfee = _.extend(_.findWhere(data[0].results_1,{name: 'initialfee'}), {"value": -inputs.initialfee}); };
    //if(inputs.initialfee != 0){ result._1.initialfee               = _.extend(localElems['initialfee'],             {"value": -inputs.initialfee}); };
    result._1.numberpays               = _.extend(_.findWhere(data[0].results_1,{name: 'numberpays'}), {"value": helper.numberpays});
    //result._1.numberpays              = _.extend(localElems['numberpays'],              {"value": helper.numberpays});
    result._1.savingratio               = _.extend(_.findWhere(data[0].results_1,{name: 'savingratio'}), {"value": helper.savingratio});
    ...
    if(helper.irrSave !== null && isFinite(helper.irrSave)){result._1.irrSave   = _.extend(_.findWhere(data[0].results_1,{name: 'irrsave'}), {"value": helper.irrSave * 100}); };
    //if(helper.irrSave !== null && isFinite(helper.irrSave)){result._1.irrSave = _.extend(localElems['irrsave'],     {"value": helper.irrSave * 100}); };

    result._1.totalloanpay            = _.extend(_.findWhere(data[0].results_1,{name: 'totalloanpay'}), {"value": helper.totalloanpay});
    //result._1.totalloanpay          = _.extend(localElems['totalloanpay'],            {"value": helper.totalloanpay});
    result._1.totalloanwinterest      = _.extend(_.findWhere(data[0].results_1,{name: 'totalloanwinterest'}), {"value": helper.totalloanwinterest});
    //result._1.totalloanwinterest    = _.extend(localElems['totalloanwinterest'],      {"value": helper.totalloanwinterest});
    result._1.totalloan               = _.extend(_.findWhere(data[0].results_1,{name: 'totalloan'}), {"value": helper.totalloan});
    //result._1.totalloan             = _.extend(localElems['totalloan'],               {"value": helper.totalloan});
    result._1.interestloan               = _.extend(_.findWhere(data[0].results_1,{name: 'interestloan'}), {"value": helper.interestloan});
    //result._1.interestloan            = _.extend(localElems['interestloan'],            {"value": helper.interestloan});
    result._1.totalloanpays               = _.extend(_.findWhere(data[0].results_1,{name: 'totalloanpays'}), {"value": helper.totalloanpays});
    //result._1.totalloanpays           = _.extend(localElems['totalloanpays'],           {"value": helper.totalloanpays});
    result._1.termloan               = _.extend(_.findWhere(data[0].results_1,{name: 'termloan'}), {"value": helper.termloan});
    //result._1.termloan                = _.extend(localElems['termloan'],                {"value": helper.termloan});

    if(helper.irrLoan !== null && isFinite(helper.irrLoan)){result._1.irrLoan = _.extend(_.findWhere(data[0].results_1,{name: 'irrloan'}), {"value": helper.irrLoan * 100}); };
    ...
    if (helper.totalloan <= 0.004999){  // case where no loan is necessary
      ...
      result._1.totalloanwinterest = _.extend(_.findWhere(data[0].results_1,{name: 'totalloanwinterest'}), {"value": 0});
  ...
  return Calc.findByCalcname('homesave')
      .then(compute)
      .onReject(function(){
        console.log("Database read error");
        helpers.errors.set("Leider ist bei der Berechnung ein Fehler aufgetreten.",undefined , true);
...
exports.mortgage = function(inputs){
  ...
  function compute(data){
    ...
    if(helper.term * inputs.repayfreq !== inputs.term){
      // todo: check whether value in if condition makes sense (s.a. test case 8)
