...
savings.schedule = function(inputs){
  ...
  /** wrapper around scheduleTerminal for computing the principal (terminal, inflow, dynamic, interest, term given) */
  function computePrincipal(principal){
    ...
    //return self.terminal - locDyn[4][locDyn[4].length - 1] - (self.compounding - 1) * _.reduce(locDyn[3], basic.sum, 0) - helper.interestResidual;

    //return self.terminal - locDyn[4][locDyn[4].length - 1] + helper.interestResidual;
    //return self.terminal - locDyn[4][locDyn[4].length - 1]  - (self.compounding - 1) * (helper.totalInterest);
  }
