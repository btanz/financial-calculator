...
savings.schedule = function(inputs){
  var self = this;
  var helper = {};
  var scheduleTerminal, dyn, dynT, dynTermfix, dynTermfixT, j, k, l;
  /** set defaults */
  ...
  scheduleTerminal = function(principal, inflow, interest, term, dynamic, inflowfreq, interestfreq, compounding, inflowtime, termfix){
    ...
    for(i = 1, terminal = (term + termfix) * 12; i <= terminal; i++){
      ...
      if((i % (12/inflowfreq) === 0 && (inflowtime === 1) && i <= termMonth) || ((i-1) % (12/inflowfreq) === 0 && (inflowtime === 2) && (i-1) <= termMonth)) {
        /** periodical payment */
        dyn[2][k - 1] = inflow;
      } else {
        dyn[2][k - 1] = 0;
  ...
  //dynTermfix = scheduleTerminal(dyn[4][dyn[4].length - 1], 0, self.interest, self.termfix, self.dynamic, self.inflowfreq, self.interestfreq, self.compounding, self.inflowtime, dyn[0][dyn[0].length - 1]);
  //dynTermfixT = dynTermfix[0].map(function (col, i) {return dynTermfix.map(function (row) {return row[i];})});

  /** attach final fixed term */
  //Array.prototype.push.apply(dynT, dynTermfixT);
  ...
  if(!dynT[dynT.length - 1][5]){
    dynT.push([Math.ceil(this.term) + '. Jahr', dyn[1][dyn[1].length + 1 - k], _.reduce(dyn[2].slice(dyn[2].length + 1 - k, dyn[2].length), basic.sum, 0), _.reduce(dyn[3].slice(dyn[3].length + 1 - k, dyn[3].length), basic.sum, 0), dyn[4][dyn[4].length - 1],true]);
  }
