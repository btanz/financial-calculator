 ...
 [ -0.023095689, 0.047603676, -0.039105981, 0.101275597, 0.045353652, 0.026653453,
  ...
  for(i = 0; i < length; i++){
    for(j = i; j < length; j++) {
      // we add a correction factor as stats computes the empirial covariance by default
      // moreover, exploit symmetry of covariance matrix
      covariance[i][j] = stats.covariance(returns[i], returns[j]) * (returns[0].length - 1) / returns[0].length;
      covTemp[i][j] = covariance[i][j];
      if(i !== j){
        covariance[j][i] = covariance[i][j];
        covTemp[j][i]    = covariance[i][j];
  ...
  returns   = stats.transpose(returns);
  k         = returns[0].length;
  var Dmat  = covariance;
  var dvec  = stats.zeros(1, k)[0];
  var a1    = stats.ones(1, k)[0];
  var a2    = e;
  var a3    = stats.identity(k);
  var a3neg = stats.create(k, k, function retIdent(i, j) {
    return i === j ? -1 : 0;
  });
  var b3  = stats.zeros(1, k)[0];
  var Amat = rbind(a1, a2, a3, a3, a3neg);
  ...
  var bvec = [1].concat(pm).concat(b3).concat(lows).concat(highs);
