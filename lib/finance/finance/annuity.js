...
annuity.schedule = function(inputs){
  ...
  if(this.mode === 1){
    ...
    if(this.finalvals){
      dynT.push(['Summe', dyn[1][0], _.reduce(dyn[2], basic.sum, 0), _.reduce(dyn[3], basic.sum, 0), _.reduce(dyn[4], basic.sum, 0), dyn[5][dyn[5].length - 1], true]);
    }

    return {
      schedule:   dynT,
      residual:   dyn[5][dyn[5].length - 1],
      totalrepay: _.reduce(dyn[2], basic.sum, 0),
