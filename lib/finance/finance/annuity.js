...
annuity.schedule = function(inputs){
  ...
  var scheduleResidual, viewPrincipal = this.principal, specialRepays;
  ...
  scheduleResidual = function(principal, repay, term, repayfreq, interest, noamortization, specialrepay){
    ...
    var i, j = 1, terminal;   /** i counts rows, j counts periods */
    var dyn = []; dyn[0] = []; dyn[1] = []; dyn[2] = []; dyn[3] = []; dyn[4] = []; dyn[5] = []; dyn[6] = []; dyn[7] = [];

    for(i = 1, terminal = term * repayfreq; i <= terminal; i++){
      dyn[0][i-1] = j * 12 / repayfreq;                                 /** current month */
      ...
      if(specialrepay[j]){
        i += 1;
        dyn[0][i-1] = String(j + ' (ST)');
        dyn[1][i-1] = (i-1 === 1) ? principal : dyn[5][i-2];
        dyn[3][i-1] = 0;
        dyn[2][i-1] = Math.min(dyn[1][i-1], specialrepay[j]);
        dyn[4][i-1] = Math.min(dyn[1][i-1], specialrepay[j]);
        dyn[5][i-1] = Math.max(0, dyn[1][i-1] - dyn[4][i-1]);
        dyn[6][i-1] = false;                                              /** annual value indicator */
        dyn[7][i-1] = true;                                              /** special repayment indicator; allows string in dyn[0] output */
        terminal += 1;
      }
    ...
    if(this.annualvals){
      ...
      dyn[0].forEach(function(val, ind){
        ...
        if((val) % 12 === 0 && !dyn[7][ind + 1]){  /** CASE: NO eop special reayment */
          dynT.splice(ind + j,    0,[val/12, dyn[1][ind + 1 - k ], _.reduce(dyn[2].slice(ind + 1 - k, ind + 1), basic.sum, 0), _.reduce(dyn[3].slice(ind + 1 - k, ind + 1), basic.sum, 0), _.reduce(dyn[4].slice(ind + 1 - k, ind + 1), basic.sum, 0),dyn[5][ind],true]);
          j += 1;
          k = 0;
        } else if ((val) % 12 === 0 && !dyn[7][ind]){  /** CASE: eop special reayment */
          dynT.splice(ind + j + 1,0,[val/12, dyn[1][ind + 1 - k ], _.reduce(dyn[2].slice(ind + 1 - k, ind + 2), basic.sum, 0), _.reduce(dyn[3].slice(ind + 1 - k, ind + 2), basic.sum, 0), _.reduce(dyn[4].slice(ind + 1 - k, ind + 2), basic.sum, 0),dyn[5][ind+1],true]);
          j += 1;
          k = -1;
        }
        k += 1;
