  ...
  scheduleResidual = function(principal, repay, term, repayfreq, interest, noamortization, specialrepay){
    ...
    for(i = 1, terminal = term * 12; i <= terminal; i++){
      ...
      if(i % (12/repayfreq) === 0) {
        ...
        /** periodical interest */
        if(specialRepays.ind){ /** case with paytime-incongruent special repayments that require interest treatment */
      ...
      if(specialrepay[i]){
        ...
        if(i % (12 / repayfreq) !== 0){
          ...
          specialRepays.fullPeriodInterest =  (k === 1) ? principal : dyn[5][k - 2] * (interest / repayfreq);
          /** compute time to next paytime */
          specialRepays.timeToNextPays = (12 / repayfreq) - i % (12 / repayfreq);
          /** compute interest for full period */
          specialRepays.interest = specialRepays.fullPeriodInterest - (specialRepays.timeToNextPays / 12) * interest * specialrepay[i];
        }
        dyn[0][k - 1] = String(i + ' (ST)');
        dyn[1][k - 1] = (k - 1 === 1) ? principal : dyn[5][k - 2];
        dyn[3][k - 1] = 0;
