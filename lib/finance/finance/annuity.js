...
annuity.schedule = function(inputs){
  ...
  scheduleResidual = function(principal, repay, term, repayfreq, interest, noamortization, specialrepay){
    ...
    //var cfDelay = self.repaymentfree ? self.repaymentfreeterm * repayfreq : 0; /** delay of cf's due to paymentfree periods */
    ...
    for(i = 1, terminal = term * 12; i <= terminal; i++){
      ...
      if(i % (12/repayfreq) === 0) {
        ...
        // todo: deal with case where interestfree period is not multiple of payperiod
        if (noamortization) {
          /** periodical annuity */
          dyn[2][k - 1] = dyn[3][k - 1];
  ...
  if(this.mode === 1){
    ...
    console.log(helper.irrprincipal);
    console.log(cf);
