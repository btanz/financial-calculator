  ...
  scheduleResidual = function(principal, repay, term, repayfreq, interest, noamortization, annualrepay){
    ...
    if(basic.round(annualrepay,2) > 0){
      ...
      dyn[0].forEach(function(val, ind){
        if((val) % 6 === 0){
          dynT.splice(ind + j,0,[val + ' (ST)',dyn[5][ind],annualrepay,0,annualrepay,dyn[5][ind]-annualrepay,false,true]);
          j += 1;
        }
      });
    ...
    if(this.annualvals){
      ...
      dyn[0].forEach(function(val, ind){
        ...
        if((val) % 12 === 0 && !dyn[7][ind + 1]){  /** CASE: NO eop special reayment */
          ...
          dynT.splice(ind + j + 1,0,[val/12, dyn[1][ind + 1 - k ], _.reduce(dyn[2].slice(ind + 1 - k, ind + 2), basic.sum, 0), _.reduce(dyn[3].slice(ind + 1 - k, ind + 2), basic.sum, 0), _.reduce(dyn[4].slice(ind + 1 - k, ind + 2), basic.sum, 0),dyn[5][ind],true]);
          j += 1;
          k = -1;
        }
        k += 1;
